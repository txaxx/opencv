...
int main(){
	...
	cv::GaussianBlur(binary, binary,cv::Size(3,3),0,0); //高斯平滑
	
	//形态学操作（此处效果不好，只用来展示加强理解）
	cv::erode(binary, binary,cv::Mat(),cv::Point(-1,-1),1);
	cv::dilate(binary, binary,cv::Mat(),cv::Point(-1,-1),1);
	
	//--4.寻找轮廓
	std::vector<std::vector<cv::Point> > contours;
	//contours这里出现异常，可以手动分配内存空间大小contours(10000)
	cv::Mat binary_copy; //因为findcontours函数会改变输入的图像，所以复制一个图像作为函数的输入
	binary.copyTo(binary_copy);
	cv::findContours(binary_copy,contours,CV_RETR_EXTERNAL/*获取外轮廓*/,CV_CHAIN_APPROX_NONE/*获取每个轮廓的每个像素*/, cv::Point());
